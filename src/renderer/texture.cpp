...
Texture::Texture(Renderer* iRenderer, const Config& iConfig, const void* iPixelData, VkDeviceSize iSize)
    : mRenderer(iRenderer), mConfig(iConfig) {
  NE_ASSERT(mRenderer, "Renderer must not be null");
  NE_ASSERT(mConfig.width > 0 && mConfig.height > 0, "Texture dimensions must be greater than 0");
...
std::unique_ptr<Texture> Texture::createWhite1x1(Renderer* iRenderer, std::string iDebugName) {
  ...
  return std::make_unique<Texture>(iRenderer, config, &whitePixel, sizeof(whitePixel));
}

std::unique_ptr<Texture> Texture::createFlatNormal1x1(Renderer* iRenderer, std::string iDebugName) {
  // Tangent-space flat normal is (0, 0, 1). Encoded into [0, 255] UNORM:
  const uint8_t normalPixel[4] = {128, 128, 255, 255};
  Config config{
      .width = 1,
      .height = 1,
      .format = VK_FORMAT_R8G8B8A8_UNORM, // Normals are linear UNORM, NOT sRGB
      .usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT,
      .mipLevels = 1,
      .debugName = std::move(iDebugName),
  };
  return std::make_unique<Texture>(iRenderer, config, normalPixel, sizeof(normalPixel));
}

std::unique_ptr<Texture> Texture::createBlack1x1(Renderer* iRenderer, std::string iDebugName) {
  ...
  return std::make_unique<Texture>(iRenderer, config, blackPixel, sizeof(blackPixel));
}
...
void Texture::initResources() {
  ...
  NE_LOG("Allocated Texture{}: Extent {}x{} | Format: {}",
         mConfig.debugName.empty() ? "" : std::format(" '{}'", mConfig.debugName),
...
void Texture::transitionLayout(VkCommandBuffer iCommandBuffer, VkImageLayout iNewLayout,
                               VkAccessFlags2 iDstAccessMask, VkPipelineStageFlags2 iDstStageMask) {
...
void Texture::transitionLayout(VkImageLayout iNewLayout, VkAccessFlags2 iDstAccessMask, VkPipelineStageFlags2 iDstStageMask) {
  if (mCurrentLayout == iNewLayout) {
    return;
  }
...
void Texture::uploadData(const void* iPixelData, VkDeviceSize iSize) {
  NE_ASSERT(iPixelData && iSize > 0, "Invalid pixel data for texture upload");

  Buffer stagingBuffer(mRenderer, iSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                       VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                       mConfig.debugName.empty() ? "Texture_Staging" : mConfig.debugName + "_Staging");
  stagingBuffer.writeToBuffer(iPixelData, iSize, 0);

  VkCommandBuffer cmd = mRenderer->beginOneTimeCommand();

  // 1. Transition image to TRANSFER_DST_OPTIMAL
  transitionLayout(cmd, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_2_TRANSFER_WRITE_BIT,
