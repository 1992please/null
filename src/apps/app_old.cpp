...
AppOld::AppOld()
    : mWindow(mWidth, mHeight, "Hello Vulkan"), mDevice(mWindow), mSwapChain(mDevice, mWindow.getExtent()) {
...
AppOld::~AppOld() { vkDestroyPipelineLayout(mDevice.device(), mPipelineLayout, nullptr); }
...
void AppOld::createPipeline() {
  ...
  mPipeline = std::make_unique<Pipeline>(mDevice, "compiled_shaders/triangle.vert.spv",
                                         "compiled_shaders/triangle.frag.spv", pipelineConfig);
}
